...
router.post('/', async (req: Request, res: Response) => {
  try {
    const { cursoId, titulo, descripcion, contenido, recompensaXP, ordenIndice } = req.body;
    
    if (!cursoId || !titulo) {
      return res.status(400).json({
        success: false,
        message: 'Faltan campos requeridos: cursoId, titulo',
    ...
    const result = await databaseService.executeStoredProcedure(
      'SP_Leccion_Crear',
      { cursoId, titulo, descripcion, contenido, recompensaXP, ordenIndice }
    );
    ...
    if (result.recordset[0].Resultado === 'Exito') {
      res.status(201).json({
        ...
        data: {
          leccionId: result.recordset[0].LeccionId
        }
      });
    } else {
      res.status(400).json({
 ...
 * Crear nueva lección para un curso específico
 * Requiere autenticación
 */
router.post('/courses/:id/lessons', async (req: Request, res: Response) => {
  try {
    const cursoId = parseInt(req.params.id);
    const { titulo, descripcion, contenido, recompensaXP, ordenIndice } = req.body;
